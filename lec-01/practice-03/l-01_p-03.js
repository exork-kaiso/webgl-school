...
class App3 {
	...
	*/
	...
	init() {
		...
		const BOX_COUNT = 100;
		const BOX_TRANSFORM_SCALE = 5.0;
		this.boxGeometry = new THREE.BoxGeometry(0.1, 0.1, 0.1);
		this.boxArray = [];
		for( let i = 0; i < BOX_COUNT; ++i){
			const box = new THREE.Mesh(this.boxGeometry, this.material);
			box.position.x = (Math.random() * 2.0 - 1.0) * BOX_TRANSFORM_SCALE;
			box.position.y = (Math.random() * 2.0 - 1.0) * BOX_TRANSFORM_SCALE;
			box.position.z = (Math.random() * 2.0 - 1.0) * BOX_TRANSFORM_SCALE;
			this.scene.add(box);
